...
vim.api.nvim_create_user_command("LSPStop ", function(opts)
  if opts.args then
    ...
    F.notify("INFO", "[LSP] Stopping client " .. opts.args .. ".")
  end
...
local restart = function(client_id)
  ...
  F.notify("INFO", "[LSP] Restart: Attached buffers: " .. vim.fn.string(attached_buffers))
  LSP.stop(client, {
    on_close = function(c)
...
vim.api.nvim_create_user_command("LSPRestart ", function(opts)
  if opts.args then
    ...
    F.notify("INFO", "[LSP] Restarting client " .. opts.args .. ".")
  end
...
local buf_active_clients = function()
  ...
  if tbl_langs == "" then
    F.notify("INFO", "[LSP] No active clients.")
  else
    F.notify("INFO", "[LSP] Active clients: " .. tbl_langs)
  end
end
